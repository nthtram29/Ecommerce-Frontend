      ...
      if(name && address && phone){
        mutationUpdate.mutate({ id: user?.id, token: user?.access_token, ...stateUserDetails }, {
          onSuccess: () => {
            ...
            setIsOpenModalUpdateInfo(false)
            fetchGetDetailsUser(user?.id, user?.access_token)
          }
        })
      }
    }
  ...
  return (
    ...
    <div style={{ background: `rgb(255,253,255)`, background: `linear-gradient(0deg, rgba(255,255,255,1) 0%, rgba(255,255,255,1) 98%, rgba(247,244,244,1) 99%, rgba(217,201,201,1) 100%)`, with: '100%', height: '100%' }}>
      <div style={{ height: '100%', width: '1100px', margin: '0 auto' }}>
        <h3 style={{ fontWeight: '500', margin: '0', padding: '20px 0' }}>Giỏ hàng</h3>
          ...
          <WrapperLeft>
            ...
            <WrapperStyleHeader>
              <span style={{ display: 'inline-block', width: '390px' }}>
                ...
                <span style={{color: '#fff'}}> Tất cả ({order?.orderItems?.length} sản phẩm)</span>

              </span>
              <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{color: '#fff'}}>Đơn giá</span>
                <span style={{color: '#fff'}}>Số lượng</span>
                <span style={{color: '#fff'}}>Thành tiền</span>
                <DeleteOutlined style={{ cursor: 'pointer' , color: '#fff'}} onClick={handleRemoveAllOrder} />
              </div>
            </WrapperStyleHeader>
            <WrapperListOrder>
            ...
            <div style={{ width: '100%' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' , padding: '5px 10px'}}>
                <span>Tổng tiền</span>
                  ...
                  <span style={{ color: 'rgb(254, 56, 52)', fontSize: '14px', fontWeight: 'bold' }}>{convertPrice(totalPriceMemo)}</span>
                  </div>
